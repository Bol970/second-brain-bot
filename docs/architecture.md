...
Задача - это обычный `item` с типом `task` плюс строка в `reminders` для статуса и необязательного `due_at`.
...
Для фотографий Telegram уже отдаёт несколько вариантов разного размера. Бот берёт самый большой вариант ниже целевого размера, а не полный оригинал. На Cloudflare он затем пробует трансформацию изображения через `cf.image` перед сохранением в R2.

Настройки R2 - это локальная конфигурация деплоя:
...
- R2-хранилище медиа включено, но за ним нужно следить: сверх бесплатного лимита R2 платный.
